...
def render_threejs_scene(poem_title, poem_content, poem_translate=""):
    ...
    has_river = has_any(all_text, ["黄河", "长江", "江", "河", "海", "流水", "波", "浪"]) or has_any(
    ...
    has_animal = has_any(all_text, ["马", "鸟", "黄莺", "鹤"]) or has_any(text_lower, ["horse", "bird", "oriole", "crane"])
    has_people = has_any(all_text, ["美人", "妾", "人", "客", "翁", "君", "童子"]) or has_any(
        text_lower, ["lady", "person", "guest", "old man", "gentleman", "boy"]
    )
    ...
    animal_bird = has_any(all_text, ["鸟", "黄莺", "鹤"])
    animal_crane = has_any(all_text, ["鹤"])
    people_lady = has_any(all_text, ["美人", "妾"])
    people_old = has_any(all_text, ["翁"])
                    ...
                    try {{
                        ...
                        function addPerson(x, z, variant = 'normal') {{
                            ...
                            const body = new THREE.Mesh(
                                new THREE.CylinderGeometry(0.62, 0.82, variant === 'child' ? 1.9 : 2.5, 14),
                        ...
                        function addHorse(x, z) {{
                            ...
                            const group = new THREE.Group();
                            const body = new THREE.Mesh(
                                new THREE.BoxGeometry(3.4, 1.5, 1.15),
                            ...
                            const neck = new THREE.Mesh(
                                new THREE.BoxGeometry(0.95, 1.2, 0.72),
                                new THREE.MeshStandardMaterial({{ color: horseColor }})
                            );
                            neck.position.set(1.65, 0.75, 0);
                            group.add(neck);
                        ...
                        function addBoat(x, z) {{
                            ...
                            const hull = new THREE.Mesh(
                                new THREE.CylinderGeometry(0.95, 0.95, 6.8, 12),
                            ...
                            const mast = new THREE.Mesh(
                                new THREE.CylinderGeometry(0.09, 0.09, 3.2, 10),
                                new THREE.MeshStandardMaterial({{ color: 0x6f573f }})
                            );
                            mast.position.set(0.25, 0.85, 0);
                            group.add(mast);
                        ...
                        function addCart(x, z) {{
                            ...
                            const cart = new THREE.Mesh(
                                new THREE.BoxGeometry(3.2, 1.3, 1.8),
                                new THREE.MeshStandardMaterial({{ color: 0x6f4a2e }})
                            );
                            cart.position.set(0, -0.15, 0);
                            group.add(cart);
                            ...
                            for (const dx of [-0.7, 0.7]) {{
                                for (const dz of [-0.7, 0.7]) {{
                                    const wheel = new THREE.Mesh(new THREE.CylinderGeometry(0.45, 0.45, 0.16, 14), wheelMat);
                                    wheel.rotation.x = Math.PI / 2;
                                    wheel.position.set(dx * 1.2, -0.9, dz * 0.55);
                                    group.add(wheel);
                                    wheels.push(wheel);
                                }}
                            }}
...
st.markdown("<div class='section-label'>scene (three.js)</div>", unsafe_allow_html=True)
